// build-geo-posts-invoice-reminder.mjs
// Reads GEO article markdown files from opc-doc (invoice-reminder), extracts frontmatter + JSON-LD +
...
const ARTICLES_DIR = 'E:/AgentCPM/07_一人公司出海项目/opc-doc/outputs/07-conversion/articles-invoice-reminder'
const OUT_TS = 'E:/AgentCPM/07_一人公司出海项目/12_Micro_SaaS出海/invoice-reminder/data/geoPosts.ts'
// NOTE: Vercel alias is NOT invoice-reminder.lxsaihub.com — after first deploy, replace both PROD_URL and
// LIVE_DOMAIN with the real alias (e.g. invoice-reminder-<random>.vercel.app) and re-run this script + redeploy.
const PROD_URL = 'https://invoice-reminder.lxsaihub.com'
const LIVE_DOMAIN = 'https://invoice-reminder.lxsaihub.com'
...
function parseCta(rest) {
  ...
  if (!line) return ''
  const text = line.replace(/\[WAITLIST_URL\]/g, `<a href="${PROD_URL}" class="underline font-semibold">try Dunner</a>`)
  return '<p>' + inline(text) + '</p>'
}

function fixDomain(obj) {
  return JSON.parse(JSON.stringify(obj).replace(/https:\/\/invoice-reminder\.vercel\.app/g, LIVE_DOMAIN))
}
...
for (const f of files) {
  ...
  const description = (blogPosting.description || '').replace(/https:\/\/invoice-reminder\.vercel\.app/g, LIVE_DOMAIN)
  posts.push({
    slug: fm.slug,
    ...
    description,
    html: mdToHtml(body).replace(/https:\/\/invoice-reminder\.vercel\.app/g, LIVE_DOMAIN),
    faq: parseFaq(faq),
...
const ts = `// AUTO-GENERATED by scripts/build-geo-posts-invoice-reminder.mjs — do not edit by hand.
